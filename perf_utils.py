...
def get_perf_metrics(rets, prds_in_yr=252,mean_mode="gm"):
    ...
    return(ann_ret,ann_vol,sharpe,mdd,dd_start,dd_end, -var*np.sqrt(prds_in_yr), -expected_short_fall*np.sqrt(prds_in_yr),tail_risk*np.sqrt(prds_in_yr))
...
def get_perf_summary(df_ret,prds_in_yr=252, mean_mode="gm"):
    ...
    for start_name in df_ret.columns.values:
        (ann_ret,ann_vol,sharpe,mdd,dd_start,dd_end, var, expected_short_fall,tail_risk) = get_perf_metrics(df_ret[start_name], prds_in_yr=prds_in_yr, mean_mode=mean_mode)
        df_perf.ix[start_name,:] = (ann_ret*100,ann_vol*100,sharpe,mdd*100,dd_start, dd_end, var*100, expected_short_fall*100,tail_risk*100)
